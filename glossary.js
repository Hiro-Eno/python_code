...
{target:`df.max()`, content:`各列・各行の最大値を取得。<br>df.***() → 各列の最大値<br>df['A'].***() → 'A'列の最大値<br>df.***(axis=1) → 各行の最大値<br>df.loc['a'].***() → 'a'行の最大値`},
...
{target:`df.loc[]`, content:`データフレームでラベルに基づいて行を選択。<br>df.***[1, '2nd'] → 12<br>df.***[1, :] → 1st 2,&emsp;2nd 12<br>df_select = df.***[df.groupby('A')['B'].idxmax()] → 'A'列でグルーピングした中で'B'列が最大値のデータを抽出`},
