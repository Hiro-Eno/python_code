...
`use strict`
const glossary = [
{target:`if`, content:`条件に基づいてプログラムの特定の部分を実行するための制御構造<br>** y > 10:<br>&emsp;実行1<br>elif y > 5:<br>&emsp;実行2<br>else:<br>&emsp;実行3<br>三項演算子 x = 真の場合の値 *** 条件 else 偽の場合の値<br>'even' *** x % 2 == 0 else 'odd'<br>'negative' *** x < 0 else 'positive' *** x > 0 else 'zero'`},
...
{target:`in`, content:`xがyに含まれているとTrue、含まれていないとFalseを返すように、シーケンスに指定した要素が含まれるか判定。<br>1 *** [0, 1, 2] → True<br>100 *** [0, 1, 2] → False<br>1 *** (0, 1, 2) → True<br>1 *** {0, 1, 2} → True<br>1 *** range(3) → True<br>dic = {'A': 101, 'B': 102, 'C': 103}<br>'A' *** dic → True<br>101 *** dic → False<br>101 *** dic.values() → True<br>('A', 101) *** dic.items() → True<br>('A', 102) *** dic.items() → False<br>'a' *** 'abc' → True<br>[1, 2] *** [1, 2, 3] → False<br>set([1, 2]) <= set([1, 2, 3]) → True`},
{target:`not in`, content:`シーケンスに指定した要素が含まれていないかを判定。<br>*** 1 *** [1, 2, 3] → False<br>*** 10 *** [1, 2, 3] → True`},
{target:`is`, content:`オブジェクトが同じかどうかを判定。2つのオブジェクトが同一であればTrue、同一でなければFalseを返す。<br>[1, 2, 3] *** [1, 2, 3] → False<br>[1, 2, 3] == [1, 2, 3] → True`},
{target:`for`, content:`シーケンス（リスト、タプル、文字列など）内の各要素に対して反復処理を行うための制御構造<br>*** i in range(5):<br>&emsp;print(i) → 0, 1, 2, 3, 4<br>*** i in array:<br>&emsp;print(i) → a, b, c<br>*** i in dic:<br>&emsp;print(i, dic[i]) → A, 101&emsp;B, 102&emsp;C, 103<br>*** i in df['1st']:<br>&emsp;print(i) → 1, 2, 3<br>*** i, j in zip(df.iloc[:, 0], df.iloc[:, 1]):<br>&emsp;print(i, j) → 1, 11&emsp;2, 12&emsp;3, 13<br>*** index, row in df.iterrows():<br>&emsp;print(index) → 0, 1, 2<br>&emsp;print(row.values) → [1, 11], [2, 12], [3, 13]<br>*** row in df.itertuples():<br>&emsp;print(row[1]) → 1, 2, 3<br>*** row in df.itertuples(index=False, name=None):<br>&emsp;print(row) →(1, 11), (2, 12), (3, 13)`},
{target:`while`, content:`指定された条件がTrueである限り、一連の文を繰り返し実行するための制御構造。<br>*** count < 5:<br>&emsp;print(count)<br>&emsp;count += 1`},
{target:`else`, content:`ループ処理を正常に終了した時に実行する処理。<br>for i in array:<br>&emsp;print(i)<br>***:<br>&emsp;print('END') → a, b, c, END`},
...
{target:`continue`, content:`条件に該当する場合、その処理をスキップして次の処理に移る。<br>for num in [1, 2, 3.14, 4, 5]:<br>&emsp;if not isinstance(num, int):<br>&emsp;&emsp;print(f'スキップ {num}は整数でない')<br>&emsp;&emsp;continue<br>&emsp;print(num)<br>→ 1, 2, スキップ 3.14は整数でない, 4, 5`},
...
{target:`class`, content:`クラスを定義。<br>*** message(): → クラス名=message<br>&emsp;def hello(self): → メソッド名=hello<br>&emsp;print('Hello!') → 実行したい処理=「Hello!」を出力<br>mess = message() → インスタンス名=mess<br>mess.hello() → 呼び出し`},
{target:`try`, content:`例外（実行中に検出されたエラー）をキャッチして処理する。例外が発生しても途中で終了させずに処理を継続できる。<br>***:<br>&emsp;print(1 / 0)<br>except ZeroDivisionError:<br>&emsp;print('Error')`},
{target:`except`, content:`例外の処理を定義。<br>try:<br>&emsp;print(1 / 0)<br>*** ZeroDivisionError:<br>&emsp;print('Error')`},
{target:`finally`, content:`例外の発生にかかわらず実行される。<br>try:<br>&emsp;print(1 / 0)<br>except ZeroDivisionError:<br>&emsp;print('Error')<br>***:<br>&emsp;print('finish') → 常に最後に'finish'と表示される`},
{target:`del`, content:`リスト内の特定の位置にある要素を削除。<br>array = ['a', 'b', 'c']<br>*** array[1] → ['a', 'c']`},
{target:`アンパック（*）`, content:`リスト等を展開。新しいリストへのコピーなどにも使える。<br>new_array = [***array]（新しいリストにコピー）<br>new_array = [***array, 'd'] → ['a', 'b', 'c', 'd']<br>[***dic] → ['A', 'B', 'C']（キーリストを取得）<br>[***df] → ['1st', '2nd']（カラムリストを取得）`},
...
{target:`bool()`, content:`真偽値を返す。<br>***(0 == 0) → True<br>***(0) → False`},
...
{target:`dir()`, content:`オブジェクトの属性リスト。<br>***(obj)<br>import math<br>***(math)`},
...
{target:`list()`, content:`リストを生成。<br>***((1, 2, 3)) → [1, 2, 3]<br>***(array) → ['a', 'b', 'c']（新しいリストにコピー）<br>***(dic) → ['A', 'B', 'C']<br>***(df) → ['1st', '2nd']<br>***(df.columns) → ['1st', '2nd']<br>`},
{target:`locals()`, content:`現在のローカルスコープにおけるシンボルテーブルを辞書として返す。ローカルスコープは、関数やメソッドの中で定義された変数や関数が存在するスコープ<br>***()`},
{target:`map()`, content:`指定された関数をイテラブルの各要素に適用して、その結果からなるイテレータを生成。<br>list(***(lambda x: x * 2, [1, 2, 3])) → [2, 4, 6]`},
{target:`max()`, content:`最大の要素を返す。<br>***(4, 7, 2) → 7`},
{target:`memoryview()`, content:`指定されたバッファプロトコルに対するビューを提供。バッファプロトコルは、データのメモリ上のバッファを効率的に共有するためのプロトコル。<br>***(b'hello') → <memory at 0x000001883623FE80>`},
...
{target:`pd.DataFrame()`, content:`データフレームを生成。<br>df = ***.***(data)<br>data =[[1, 2, 3], [4, 5, 6]]<br>df_data = pd.DataFrame(data) → 1, 2, 3&emsp;4, 5, 6`},
{target:`df.head()`, content:`データフレームの先頭の行を表示。<br>***.***()<br>***.***(10)`},
{target:`df.tail()`, content:`データフレームの末尾の行を表示。<br>***.***()<br>***.***(10)`},
{target:`df.info()`, content:`データフレームの情報表示。<br>***.***()`},
{target:`df.describe()`, content:`データフレームの統計的な概要を表示。<br>***.***()`},
{target:`df.shape`, content:`データフレームの形状を表示。<br>***.*** → (3, 2)`},
{target:`df.columns`, content:`データフレームのカラム名を取得。<br>list(***.***) → ['1st', '2nd']`},
...
{target:`df.dropna()`, content:`データフレームの欠損値を含む行を削除。<br>df = ***.***()`},
{target:`df.fillna(value)`, content:`データフレームの欠損値を指定の値で補完。<br>***.***(0)<br>***.***(df.mean(numeric_only=True)) → 平均値で補完`},
{target:`df.isin()`, content:`データフレームに要素が指定した値またはリストに含まれるかどうかを確認。該当するデータを抽出できる。<br>data = {'A': [1, 2, 3, 4, 5],<br>&emsp;'B': ['apple', 'banana', 'orange', 'apple', 'grape'],<br>&emsp;'C': [0.5, 0.7, 0.2, 0.9, 0.4]}<br>df = pd.DataFrame(data)<br>result = result = df[df['A'].***([2, 4, 6])] →<br>&emsp;A&emsp;B&emsp;C<br>1&emsp;2&emsp;banana&emsp;0.7<br>3&emsp;4&emsp;apple&emsp;0.9`},
{target:`df.groupby()`, content:`データフレームでグループ化して統計量を計算。<br>grouped_data = ***.***('column_name').mean()`},
{target:`df.size()`, content:`データフレーム（DataFrame）の要素の総数を取得。groupby()と組合せることで各グループ内のデータ数をカウントできる。<br>df_num = pd.DataFrame(df.groupby('group_column').***(), columns=['num'])`},
...
{target:`df.filter()`, content:`データフレームの特定の条件に合致する行を選択。<br>***.***(items=['1st']) → 1, 2, 3<br>***.***(like='n', axis=1) → 11, 12, 13`},
{target:`df.query()`, content:`データフレームの条件式に合致する行を選択。<br>***.***('column > 10')<br>***.***(''2nd' > 11') → 2, 12&emsp;3, 13<br>df[df['2nd'] > 11] → 2, 12&emsp;3, 13`},
{target:`df.loc[]`, content:`データフレームでラベルに基づいて行を選択。<br>***.***[1, '2nd'] → 12<br>***.***[1, :] → 2, 12`},
{target:`df.iloc[]`, content:`データフレームでインデックスに基づいて行を選択。<br>***.***[1, 1] → 12<br>***.***[1:, 1] → 12, 13`},
{target:`df.at[]`, content:`ラベルとカラム名を指定して値を取得。<br>***.***[1, '2nd'] → 12`},
{target:`df.iat[]`, content:`インデックスとカラム番号を指定して値を取得。<br>***.***[1, 1] → 12`},
{target:`df.to_dict()`, content:`データフレームを辞書形式に変換。<br>***.***() → {'1st': {0: 1, 1: 2, 2: 3}, '2nd': {0: 11, 1: 12, 2: 13}}<br>***.***(orient='list') → {'1st': [1, 2, 3], '2nd': [11, 12, 13]}`},
{target:`pd.read_excel()`, content:`Excelファイルからデータフレームを読み込む。<br>df = ***.***('input_file.xlsx', sheet_name='Sheet1')<br>df = ***.***('input_file.xlsx', sheet_name=[0, 1, 4])`},
{target:`pd.ExcelWriter()`, content:`ExcelWriterオブジェクトを作成。<br>writer = ***.***('output_file.xlsx', engine='openpyxl'):`},
{target:`df.to_excel()`, content:`データフレームをExcelファイルに書き込む。<br>***.***(writer, sheet_name='Sheet1', index=False, header=True)`},
...
{target:`np.tolist()`, content:`NumPy配列をリストに変換。<br>arr = np.array([[1, 2, 3], [4, 5, 6]])<br>list = arr.***() →[[1, 2, 3], [4, 5, 6]]`},
...
{target:`np.max()`, content:`NumPy配列の最大値を取得。<br>arr = np.array([[1, 2, 3], [4, 5, 6]])<br>max_value = ***.***(arr)`},
{target:`np.min()`, content:`NumPy配列の最小値を取得。<br>arr = np.array([[1, 2, 3], [4, 5, 6]])<br>min_value = ***.***(arr)`},
