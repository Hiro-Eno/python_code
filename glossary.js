...
`use strict`
const glossary = [
{target:`if`, content:`条件に基づいてプログラムの特定の部分を実行するための制御構造。<br>num = 10<br>** num < 0:<br>&emsp;print('negative')<br>elif num > 0:<br>&emsp;print('positive')<br>else:<br>&emsp;print('zero')<br><br>三項演算子<br>条件が真の場合の値 *** 条件 else 条件が偽の場合の値<br>'even' *** num % 2 == 0 else 'odd' → 'even'<br>'negative' *** x < 0 else 'positive' *** x > 0 else 'zero' → 'positive'`},
...
{target:`not in`, content:`シーケンスに指定した要素が含まれていないかを判定。<br>*** 1 *** [1, 2, 3] → False<br>*** 7 *** [1, 2, 3] → True`},
...
{target:`map()`, content:`指定された関数をイテラブルの各要素に適用して、その結果からなるイテレータを生成。<br>list(***(lambda x: x * 2, [1, 2, 3])) → [2, 4, 6]<br>array = [1, 2, 3]<br>list(map(lambda x: x*2, array)) → [2, 4, 6]`},
...
{target:`pd.DataFrame()`, content:`データフレームを生成。<br>df = ***.***(data)<br>data =[[1, 2, 3], [4, 5, 6]]<br>df_data = ***.***(data, columns=['A', 'B', 'C']) →<br>&emsp;A&emsp;B&emsp;C<br>0&emsp;1&emsp;2&emsp;3<br>1&emsp;4&emsp;5&emsp;6`},
...
{target:`df.index`, content:`データフレームのインデックス名を取得。<br>list(df.***) → [0, 1, 2]`},
{target:`df.columns`, content:`データフレームのカラム名を取得。<br>list(df.***) → ['1st', '2nd']`},
...
{target:`df.fillna(value)`, content:`データフレームの欠損値を指定の値で置換。<br>df.***(0) → 0に置換<br>***.***(df.mean(numeric_only=True)) → 数値データは平均値で置換<br>df.fillna({'1st': 0, '2nd': 10}) → '1st'の列は0、'2nd'の列は10で置換`},
...
{target:`df.isin()`, content:`データフレームに要素が指定した値またはリストに含まれるかどうかを確認。該当するデータを抽出できる。<br>data = {'A': [1, 2, 3, 4, 5],<br>&emsp;'B': ['apple', 'banana', 'orange', 'apple', 'grape'],<br>&emsp;'C': [0.5, 0.7, 0.2, 0.9, 0.4]}<br>df = pd.DataFrame(data)<br>df[df['A'].***([2, 4, 6])] →<br>&emsp;A&emsp;B&emsp;C<br>1&emsp;2&emsp;banana&emsp;0.7<br>3&emsp;4&emsp;apple&emsp;0.9<br>df[df['A']>3] →<br>&emsp;A&emsp;B&emsp;C<br>3&emsp;4&emsp;apple&emsp;0.9<br>4&emsp;5&emsp;grape&emsp;0.4`},
{target:`df.groupby()`, content:`データフレームでグループ化して統計量を計算。<br>df.***('column_name').mean()<br>data = {'A': [1, 2, 3, 4, 5],<br>&emsp;'B': ['apple', 'banana', 'orange', 'apple', 'grape'],<br>&emsp;'C': [0.5, 0.7, 0.2, 0.9, 0.4]}<br>df = pd.DataFrame(data)<br>grouped_data = df.***('B').mean()→<br>&emsp;&emsp;A&emsp;C<br>B<br>apple&emsp;2.5&emsp;0.7<br>banana&emsp;2.0&emsp;0.7<br>grape&emsp;5.0&emsp;0.4<br>orange&emsp;3.0&emsp;0.2`},
...
{target:`df.merge()`, content:`データフレームの結合。<br>merged_df = pd.***(df1, df2)<br>df1.***(df2)<br>merged_df = pd.***(df1, df2, on='key_column')<br>merged_df = pd.***(df1, df2, left_on='key_column1', right_on='key_column2')`},
...
{target:`df.plot()`, content:`Pandasライブラリを使用してデータフレームを作図。<br>df.***()<br>df.***(x='column_x', y='column_y', kind='scatter')`},
{target:`df.to_csv()`, content:`データフレームをCSVファイルに保存。<br>***.***('output.csv', index=False)`},
{target:`df.items()`, content:`列名とその列のデータのタプル(column name, Series)を1列ずつ取得できる。<br>for column_name, item in df.items():<br>&emsp;print(column_name) <br>print (item) →<br>1st, 1, 2, 3, 2nd, 11, 12, 13`},
{target:`df.iterrows()`, content:`行ごとにイテレート（繰り返し処理）。各行はインデックスと行データ（Pandas Series）のタプルとして取得。for文等で利用。<br>for index, row in df.iterrows():<br>&emsp;print(index) → 0, 1, 2<br>&emsp;print(row.values) → [1, 11], [2, 12], [3, 13]`},
{target:`df.itertuples()`, content:`行ごとにイテレート（繰り返し処理）。namedtupleを返すため、列の名前を属性として直接アクセスでき、一般にiterrows()よりも高速。<br>for row in df.itertuples(index=False, name=None):<br>print(row) → (1, 11), (2, 12), (3, 13)`},
...
{target:`df.loc[]`, content:`データフレームでラベルに基づいて行を選択。<br>df.***[1, '2nd'] → 12<br>df.***[1, :] → 1st 2,&emsp;2nd 12`},
{target:`df.iloc[]`, content:`データフレームでインデックスに基づいて行を選択。<br>df.***[1, 1] → 12<br>df.***[1:, 1] → 1 12,&emsp;2 13`},
...
{target:`df.to_excel()`, content:`データフレームをExcelファイルに書き込む。<br>df.***(writer, sheet_name='Sheet1', index=False, header=True)`},
