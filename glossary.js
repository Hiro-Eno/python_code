...
{target:`df.to_csv()`, content:`データフレームをCSVファイルに保存。<br>df.***('output.csv', index=False)`},
...
{target:`np.array()`, content:`NumPy配列を生成。<br>arr = np.***([[1, 2, 3], [4, 5, 6]]) → array([[1, 2, 3], [4, 5, 6]])<br>arr[0, 1] → 2`},
{target:`np.tolist()`, content:`NumPy配列をリストに変換。<br>arr = np.array([[1, 2, 3], [4, 5, 6]])<br>list = arr.***() →[[1, 2, 3], [4, 5, 6]]`},
{target:`np.zeros()`, content:`0で初期化されたNumPy配列を生成。<br>zeros_arr = np.***((2, 3)) → array([[0., 0., 0.], [0., 0., 0.]])`},
{target:`np.ones()`, content:`1で初期化されたNumPy配列を生成。<br>ones_arr = np.***((2, 3)) → array([[1., 1., 1.], [1., 1., 1.]])`},
{target:`np.arange()`, content:`等差数列のNumPy配列を生成。<br>arange_arr = np.***(0, 10, 2) → array([0, 2, 4, 6, 8])]`},
{target:`np.linspace()`, content:`等分された数列のNumPy配列を生成。<br>linspace_arr = np.***(0, 1, 5) → array([0.  , 0.25, 0.5 , 0.75, 1.  ])`},
{target:`np.random.rand()`, content:`0から1までのランダムな数値からなるNumPy配列を生成。<br>rand_arr = np.***.***(2, 3) → array([[0.16441106, 0.7162892 , 0.21504333], [0.7562184 , 0.75095844, 0.73152783]])`},
{target:`np.random.randn()`, content:`標準正規分布に従うランダムな数値からなるNumPy配列を生成。<br>randn_arr = np.***.***(2, 3) → array([[ 0.50079985,  2.07387442,  0.20675855], [-0.99046652,  0.11597984,  1.61186648]])`},
{target:`np.shape()`, content:`NumPy配列の形状を取得。<br>arr = np.array([[1, 2, 3], [4, 5, 6]])<br>shape = np.***(arr) → (2, 3)`},
...
{target:`os.path.basename()`, content:`拡張子を含むファイル名部分の文字列を返す。<br>os.***.***(filepath) → パスからファイル名を取得`},
{target:`os.path.splitext()`, content:`最後（一番右側）のドットで分割する。os.path.basename()と組合せることで拡張子を含まないファイル名を取得できる。<br>os.***.***(os.path.basename(filepath))[0]`},
{target:`os.path.dirname()`, content:`パス文字列からフォルダ名（ディレクトリ名）を取得する。ファイルの直上のフォルダ名のみを取得したい場合はos.path.basename()と組み合わせる。<br>os.***.***(filepath) → パスからフォルダ名を取得<br>os.path.basename(os.path.***(filepath)) → ファイル直上のフォルダ名のみを取得`},
{target:`os.path.split()`, content:`ファイル名とフォルダ名（ディレクトリ名）を両方取得する。<br>dir_pair = os.***.***(filepath) → (dir, filename)のタプルが返される`},
{target:`os.path.splitext()`, content:`拡張子とそれ以外に分割されてタプルとして返される。<br>path_extension = os.***.***(filepath) → (dir+filename, extension)のタプルが返される<br>newfilepath = os.***.***(filepath)[0] + '.jpg' → 拡張子を.jpgに変更`},
{target:`os.path.join()`, content:`ファイル名とフォルダ名を結合して新たなパス文字列を作成する。<br>path = os.***.***(root_path, f'result/{file_name}/')`},
{target:`os.makedirs()`, content:`新しいディレクトリ（フォルダ）を作成する。exist_ok=Trueとすると既に存在しているディレクトリを指定してもエラーにならない。<br>os.***(path, exist_ok=True)`},
{target:`os.getcwd()`, content:`作業ディレクトリ（カレントディレクトリ）の絶対パスを返す。<br>os.***()`},
{target:`os.chdir()`, content:`作業ディレクトリ（カレントディレクトリ）を変更できる。<br>os.***(path)`},
